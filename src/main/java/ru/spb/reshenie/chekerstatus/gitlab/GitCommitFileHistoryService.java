...
public class GitCommitFileHistoryService {
    ...
    public GitCommitFileHistoryService(GitLabProperties properties,
    ...
    public GitFileHistorySyncResult synchronizeDocumentFiles(DocumentGitLink document) {
        if (!properties.isFileHistoryEnabled()) {
        ...
        List<StoredGitLabCommit> commits = gitTrackingRepository.findStoredCommits(document.getId());
    ...
    private GitFileHistorySyncResult synchronizeDocumentFilesSequentially(DocumentGitLink document) {
        ...
        for (StoredGitLabCommit commit : commits) {
            ...
            try {
                ...
                int fileLimit = properties.getMaxFilesPerCommit();
                int fileIndex = 0;
                for (GitLabCommitDiff diff : diffs) {
                    ...
                    limiter.withDbWritePermit(new Runnable() {
                        @Override
                        public void run() {
                            fileRepository.saveFileChange(change);
                        }
                    });
    ...
    private void synchronizeCommitFiles(DocumentGitLink document,
        ...
        try {
            ...
            waitForFileTasks(document, commit, fileFutures, failedFiles, firstError, errors);
        } catch (RuntimeException e) {
    ...
    private void waitForCommitTasks(DocumentGitLink document,
        ...
        for (Future<Void> future : futures) {
            try {
                ...
                throw new GitLabClientException("Interrupted while waiting for GitLab commit file history tasks", e);
    ...
    private void waitForFileTasks(DocumentGitLink document,
        ...
        for (Future<Void> future : futures) {
            try {
                ...
                cancel(futures);
                Thread.currentThread().interrupt();
                throw new GitLabClientException("Interrupted while waiting for GitLab file content tasks", e);
            } catch (ExecutionException e) {
                Throwable cause = e.getCause() == null ? e : e.getCause();
                failedFiles.incrementAndGet();
                firstError.compareAndSet(null, cause.getMessage());
    ...
    GitCommitFileChange buildFileChange(DocumentGitLink document,
        ...
        if (!properties.isFetchFileContent()) {
